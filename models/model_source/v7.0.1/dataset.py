...
class Dataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    #return np.asarray((self.data_im[index]-self.mean_image)/self.std_image),self.data_lm[index].reshape(136), self.exprs[index]
    return np.asarray((self.data_im[index]-self.mean_image)/self.std_image), (self.exprs[index] if self.exprs[index] < 5 else 4)
